...
public class Individual_937257 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (33.588480949401855)*en.energy;
				eval += (64.2785906791687)*myEnergy;
				eval += (61.15906834602356)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (62.08951473236084)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
